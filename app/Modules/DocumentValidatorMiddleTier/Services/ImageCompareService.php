...
class ImageCompareService
{
    ...
    public function gazeCompareFace(Request $request)
    {
        ...
        try {
            ...
            Log::info("ocr".config('ekyc.api_image_compare_ocr_url'));
            $nominee_face_match = $this->guzzlePostGazeCopy(config('ekyc.api_image_compare_ocr_url'),'nid/', $request->nid_front_image_path, $request->selfie_image_path);

            $faceResult = json_decode($nominee_face_match, true);
            Log::debug('get gaze response for nominee faces: ' . print_r($faceResult, true));
            
            if ($faceResult['data']['result']['final_verification_status'] == "Yes") {
                ...
                return $this->successResponse('Face compare successfully done', $faceResult['data']);
            }
