...
class CheckerController extends Controller {
    ...
    public function index(Request $request) {
        if (!auth()->user()->can('Checker Management'))
            abort(403);
        $users = User::where('type','checker')->get();
        $roles = Role::pluck('name', 'name');
        return view("User::checker.index", compact('users', 'roles'));
    }
    ...
    public function update($id, Request $request) {
        ...
        try {
            DB::begintransaction();
            $user = User::where('type', 'checker')->findOrFail($id);
            $user->name = $request->name;
            $user->mobile_no = $request->mobile_no;
            $user->email = $request->email;
            $user->save();
    ...
    public function destroy($id) {
        ...
        try {
            User::where('id', $id)->where('type', 'checker')->delete();
